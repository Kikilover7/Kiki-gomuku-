...
def paint(x, y):
    global turn, stop, x_ed, y_ed, blackwin, whitewin, peace, whofirst, AI1, AI2
...
def AI_position(event):
    global turn, whofirst
    start = time.perf_counter()
    if turn % 2 == 1:
        if var_3.get() == 0:
            
            x, y = pvp_engine.AI_2b()
           
        else:
            x, y = pvp_engine.AI_4b()
    else:
        if var_3.get() == 1:
            x, y = pvp_engine.AI_2w()
        else:
            x, y = pvp_engine.AI_4w()
    end = time.perf_counter()
    print(end - start)
    paint(x, y)   
        
def game_run():
    global turn, stop, choice_1, blackwin, whitewin, peace, whofirst
    if var_1.get() == 0:
        if stop == 0:
...
var_3 = IntVar() 
choice_pve1 = Radiobutton(window, text = "1持黑", font = ("楷体", 14, "bold"), variable = var_3, value = 0, bg = "lime")
